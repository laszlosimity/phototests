...
class CheckoutSummaryPage extends BasePage {
    ...
    async swag(needle) {
        if (typeof needle === 'string') {
            var theItems = await this.items;
            return theItems.find(cartItem => (cartItem.getText()).includes(needle));
        }
    ...
    async getSwagText(needle) {
        var firstText =  (this.title(needle)).getText();
        var secondText = (this.description(needle)).getText();
        var thirdText = (this.price(needle)).getText();
